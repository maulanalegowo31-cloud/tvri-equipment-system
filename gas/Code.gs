 ...
 * TELAH DIPERBAIKI UNTUK MENAMBAHKAN HEADER CORS
 */
function doPost(e) {
  ...
  try {
    ...
    output.appendHeader('Access-Control-Allow-Origin', 'https://maulanaleqowo31-cloud.github.io');
    ...
    // --- KODE PERBAIKAN CORS (untuk respons error) ---
    const output = ContentService
      .createTextOutput(JSON.stringify({
        success: false,
        error: errorMessage,
 ...
 * TELAH DIPERBAIKI UNTUK MENAMBAHKAN HEADER CORS DAN MENANGANI PREFLIGHT REQUEST
 */
function doGet(e) {
  // Tambahkan ini untuk menangani preflight request (OPTIONS) dari browser
  if (e.parameter && e.parameter.cors === 'true') {
    return ContentService.createTextOutput().setMimeType(ContentService.MimeType.TEXT)
      .appendHeader('Access-Control-Allow-Origin', 'https://maulanaleqowo31-cloud.github.io')
